...
#include <uns/container/span.hpp>
...
RingBuffer<uint8_t[MAX_RX_BUFFER_SIZE], 3> rxBuffer;
...
//
//    if ((startIdx = uns::indexOf(rxBuffer.indexOf(debug::MSG_START)) != -1) {
...
extern "C" void runDebugTask(const void *argument) {
    ...
    uns::UART_Receive_DMA(cfg::uart_Bluetooth, *rxBuffer.getWritableBuffer(), MAX_RX_BUFFER_SIZE);
...
//        }
        ...
        while(txBuffer.size() + LOG_MSG_MAX_SIZE <= MAX_TX_BUFFER_SIZE && isOk(uns::queueReceive(cfg::queue_Log, &txLog))) {
 ...
 */
void uns_Bluetooth_Uart_RxCpltCallback() {
