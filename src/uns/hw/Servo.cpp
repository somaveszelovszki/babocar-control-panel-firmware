...
void hw::Servo::write(angle_t _ang) {
    _ang = uns::incarcerate(_ang, this->angle_min, this->angle_max);

    if (this->_angle != _ang) {
        this->_angle = _ang;
        uint32_t pwm = uns::map(this->_angle.get<radians>(), 0.0f, PI.get<radians>(), PWM_DUTY_0, PWM_DUTY_PI);
