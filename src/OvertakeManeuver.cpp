...
OvertakeManeuver::OvertakeManeuver()
    : Maneuver(), targetSpeedSign_(Sign::POSITIVE), state_(state_t::Prepare) {
}

void OvertakeManeuver::initialize(
    const micro::CarProps& car, const micro::Sign targetSpeedSign,
    const micro::m_per_sec_t beginSpeed, const micro::m_per_sec_t straightStartSpeed,
...
void OvertakeManeuver::update(const CarProps& car, const LineInfo& lineInfo, MainLine& mainLine,
    ...
    case state_t::Prepare:
        ...
        if (car.orientedDistance >= this->prepareDistance_ &&
            car.distance - this->initialDistance_ >= this->prepareDistance_) {
...
void OvertakeManeuver::buildTrajectory(const micro::CarProps& car) {
    ...
    const meter_t fastSectionLength =
        this->sectionLength_ - distDiff - this->beginSineArcLength_ - this->endSineArcLength_;

    const radian_t forwardAngle = Sign::POSITIVE == this->targetSpeedSign_
                                      ? car.pose.angle
                                      : normalize360(car.pose.angle + PI);

    LOG_DEBUG("Overtake: start pos: ({}, {}) | forward angle: {}deg", car.pose.pos.X.get(),
              car.pose.pos.Y.get(), static_cast<degree_t>(forwardAngle).get());

    this->trajectory_.setStartConfig(Trajectory::config_t{car.pose, this->beginSpeed_},
                                     car.distance);

    this->trajectory_.appendSineArc(
        Trajectory::config_t{
            Pose{this->trajectory_.lastConfig().pose.pos +
                     vec2m{this->beginSineArcLength_, this->sideDistance_}.rotate(forwardAngle),
    ...
    this->trajectory_.appendLine(Trajectory::config_t{
        Pose{this->trajectory_.lastConfig().pose.pos +
                 vec2m{STRAIGHT_SPEED_RAMP_DIST, centimeter_t(0)}.rotate(forwardAngle),
             this->trajectory_.lastConfig().pose.angle},
        this->straightSpeed_});

    this->trajectory_.appendLine(Trajectory::config_t{
        Pose{this->trajectory_.lastConfig().pose.pos +
                 vec2m{fastSectionLength - STRAIGHT_SPEED_RAMP_DIST, centimeter_t(0)}.rotate(
                     forwardAngle),
             this->trajectory_.lastConfig().pose.angle},
        this->straightSpeed_});
    ...
    this->trajectory_.appendSineArc(
        Trajectory::config_t{
            Pose{this->trajectory_.lastConfig().pose.pos +
                     vec2m{this->endSineArcLength_, -this->sideDistance_}.rotate(forwardAngle),
                 this->trajectory_.lastConfig().pose.angle},
            this->endSpeed_},
        forwardAngle, Trajectory::orientationUpdate_t::FIX_ORIENTATION, radian_t(0), PI_2);

    this->trajectory_.appendLine(
        Trajectory::config_t{Pose{this->trajectory_.lastConfig().pose.pos +
                                      vec2m{this->endSlideDistance_, centimeter_t(0)}.rotate(
                                          forwardAngle - degree_t(40)),
                                  this->trajectory_.lastConfig().pose.angle},
                             this->endSpeed_});
}
