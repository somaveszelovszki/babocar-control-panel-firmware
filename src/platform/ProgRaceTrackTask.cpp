...
m_per_sec_t safetyCarFollowSpeed(meter_t distFromSafetyCar, bool isFast) {
    return micro::lerp(distFromSafetyCar, meter_t(0.3f), meter_t(0.8f), m_per_sec_t(0),
                       isFast ? SAFETY_CAR_FAST_MAX_SPEED : SAFETY_CAR_SLOW_MAX_SPEED);
}
...
uint32_t getFastSection(const ProgramState::Value state) {
    return getFastSection(ProgramState::Race == state            ? 1
                          : ProgramState::Race_segFast2 == state ? 2
                          : ProgramState::Race_segFast3 == state ? 3
                          : ProgramState::Race_segFast4 == state
                              ? 4
                              : micro::numeric_limits<uint32_t>::max());
}

bool shouldHandle(const ProgramState::Value state) {
    return isBtw(underlying_value(state), underlying_value(ProgramState::ReachSafetyCar),
                 underlying_value(ProgramState::Test));
}
...
class LineDetectScanRangeManager {
  public:
    bool checkEnabled(const size_t lap, const CarProps& car, const LineInfo& lineInfo) {
        const auto enable =
            lap >= 4 && lineInfo.front.lines.size() == 1 && lineInfo.rear.lines.size() == 1;
    ...
    while (true) {
        ...
        if (shouldHandle(currentProgramState)) {
            ...
            if (std::exchange(currentLap, trackController.lap()) != trackController.lap()) {
                ...
                LOG_INFO("Lap {} finished. Time: {} ms", currentLap - 1,
                         (lapStartTime - prevLapStartTime).get());
            }
            ...
            case ProgramState::ReachSafetyCar:
                ...
                if (frontDistance != centimeter_t(0) &&
                    abs(safetyCarFollowSpeed(frontDistance,
                                             trackController.isCurrentSectionFast())) <
                        abs(controlData.speed)) {
            ...
            case ProgramState::FollowSafetyCar:
                controlData.speed =
                    safetyCarFollowSpeed(frontDistance, trackController.isCurrentSectionFast());
                controlData.rampTime = millisecond_t(0);

                if (overtakeSection == trackController.sectionIndex() &&
                    (1 == trackController.lap() || 3 == trackController.lap()) &&
                    trackController.lap() != lastOvertakeLap) {
                    programState.set(ProgramState::OvertakeSafetyCar);
                    LOG_DEBUG("Starts overtake");
                } else if (car.distance - lastDistWithSafetyCar > MAX_VALID_SAFETY_CAR_DISTANCE &&
                           trackController.isCurrentSectionFast()) {
                    programState.set(ProgramState::Race);
            ...
            case ProgramState::OvertakeSafetyCar:
                if (currentProgramState != prevProgramState) {
                    ...
                    overtake.initialize(car, Sign::POSITIVE, OVERTAKE_BEGIN_SPEED,
                                        OVERTAKE_STRAIGHT_START_SPEED, OVERTAKE_STRAIGHT_SPEED,
                                        OVERTAKE_END_SPEED, OVERTAKE_SECTION_LENGTH,
                                        OVERTAKE_PREPARE_DISTANCE, OVERTAKE_BEGIN_SINE_ARC_LENGTH,
                                        OVERTAKE_END_SINE_ARC_LENGTH, OVERTAKE_SIDE_DISTANCE);
                }
            ...
            case ProgramState::Race:
                lineDetectControlData.isReducedScanRangeEnabled =
                    scanRangeManager.checkEnabled(trackController.lap(), car, lineInfo);

                if (trackController.lap() <= 3 &&
                    frontDistance < (trackController.isCurrentSectionFast()
                                         ? MAX_VALID_SAFETY_CAR_DISTANCE - centimeter_t(2)
                                         : centimeter_t(80))) {
                    programState.set(ProgramState::FollowSafetyCar);
                    LOG_DEBUG("Reached safety car, starts following");
            ...
            case ProgramState::Finish:
                if (!trackController.isCurrentSectionFast() ||
                    car.distance - trackController.getSectionStartDistance() > meter_t(2)) {
