...
void initializeVehicleCan() {
    ...
    vehicleCanFrameHandler.registerHandler(
        can::FrontLinePattern::id(), [](const uint8_t* const data) {
            reinterpret_cast<const can::FrontLinePattern*>(data)->acquire(lineInfo.front.pattern);
            lineInfoQueue.overwrite(lineInfo);
        });

    vehicleCanFrameHandler.registerHandler(
        can::RearLinePattern::id(), [](const uint8_t* const data) {
    ...
    const CanFrameIds txFilter = {can::LineDetectControl::id()};
...
extern "C" void runLineDetectTask(void) {
    ...
    while (true) {
        ...
        if (lineInfo.front.pattern != prevLineInfo.front.pattern) {
            ...
            //                to_string(lineInfo.front.pattern.type),
            //                to_string(lineInfo.front.pattern.dir),
            //                to_string(lineInfo.front.pattern.side));
        ...
        if (lineInfo.rear.pattern != prevLineInfo.rear.pattern) {
            ...
            //                to_string(lineInfo.rear.pattern.type),
            //                to_string(lineInfo.rear.pattern.dir),
            //                to_string(lineInfo.rear.pattern.side));
        ...
        if (lineDetectControlTimer.checkTimeout()) {
            ...
            vehicleCanManager.periodicSend<can::LineDetectControl>(
                vehicleCanSubscriberId, cfg::INDICATOR_LEDS_ENABLED,
                control.isReducedScanRangeEnabled ? cfg::REDUCED_LINE_DETECT_SCAN_RADIUS : 0,
                control.domain);
        }
