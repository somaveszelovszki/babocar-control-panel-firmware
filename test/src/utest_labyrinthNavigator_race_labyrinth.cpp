...
class RaceLabyrinthNavigatorTest : public LabyrinthNavigatorTest {
  public:
    RaceLabyrinthNavigatorTest() : LabyrinthNavigatorTest() { buildRaceLabyrinthGraph(graph_); }
    ...
    void SetUp() override {
        ...
        const SegmentIds forbiddenSegments = {"MP", "MQ", "NS", "OU", "PR", "TU", "UX"};

        navigator_.initialize(graph_.getVisitableSegments(), forbiddenSegments, currentSeg,
                              prevConn, laneChangeSeg, junctionBeforeLaneChange, LABYRINTH_SPEED,
...
TEST_F(RaceLabyrinthNavigatorTest, ObstacleInCrossroads) {
    ...
    navigator_.setObstaclePositions(
        {{"FG", "EG", millisecond_t(0)}, {"DI", "IL", millisecond_t(0), true}});
...
TEST_F(RaceLabyrinthNavigatorTest, KeepSpeedSignWhenObstacleEntersCrossroad) {
    ...
    // The phantom obstacle position should be ignored and the car should continue its route with
    // the same speed sign.
    navigator_.setObstaclePositions(
        {{"IN", "DI", millisecond_t(1000), false}, {"KL", "JL", millisecond_t(1000), true}});
