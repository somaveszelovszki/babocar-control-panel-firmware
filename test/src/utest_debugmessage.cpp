...
void expectEqual(const DebugMessage::CarData& expected, const DebugMessage::CarData& result) {
...
void expectEqual(const std::optional<ParamManager::NamedParam>& expected,
    ...
    std::visit(
        [&result](const auto& exp) {
            EXPECT_EQ(exp, std::get<std::decay_t<decltype(exp)>>(result->second));
        },
        expected->second);
}
...
template <typename T> void testFormat(const T& data, const char* const expected) {
    char msg[100];
...
template <typename T> void testParse(const T& expected, etl::string<512> json) {
    T result{};
...
template <typename T> void testParseFailure(etl::string<512> json) {
    T result{};
    const auto success = DebugMessage::parse(const_cast<char*>(json.c_str()), result);
...
TEST(DebugMessage, formatTrackControl) {
    const IndexedSectionControlParameters sectionControl{
        1, TrackSection::ControlParameters{
               m_per_sec_t{1.5f}, millisecond_t{300},
               std::make_pair(OrientedLine{millimeter_t{5}, radian_t(0.2)},
                              OrientedLine{millimeter_t{-5}, radian_t(-0.2)})}};
