...
void test(const Pose& initialPose, const Sign targetSpeedSign) {
    ...
    maneuver.initialize(car, targetSpeedSign, OVERTAKE_BEGIN_SPEED, OVERTAKE_STRAIGHT_START_SPEED,
                        OVERTAKE_STRAIGHT_SPEED, OVERTAKE_END_SPEED, OVERTAKE_SECTION_LENGTH,
                        OVERTAKE_PREPARE_DISTANCE, OVERTAKE_BEGIN_SINE_ARC_LENGTH,
                        OVERTAKE_END_SINE_ARC_LENGTH, OVERTAKE_SIDE_DISTANCE);

    const point2m posDiff =
        point2m{targetSpeedSign * meter_t(1), meter_t(0)}.rotate(car.pose.angle);
    ...
    const point2m expectedEndPos =
        initialPose.pos +
        (Sign::POSITIVE == targetSpeedSign ? TRAJECTORY_END_POS : TRAJECTORY_END_POS.rotate180())
            .rotate(initialPose.angle);
